...
library SafeERC20 {
     ...
     */
    function transferAndCallRelaxed(IERC1363 token, address to, uint value, bytes memory data) internal {
        if (to.code.length == 0) {
     ...
     * Reverts if the returned value is other than `true`.
     */
    function approveAndCallRelaxed(IERC1363 token, address to, uint value, bytes memory data) internal {
        if (to.code.length == 0) {
